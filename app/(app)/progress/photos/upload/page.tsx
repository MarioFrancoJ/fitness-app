  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-2xl font-bold tracking-tight text-zinc-900">Upload Progress Photo</h1><p className="mt-1 text-sm text-zinc-500">Document your transformation with a new progress photo.</p></div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
          ...
          {imageUrl ? (
            <div className="mb-4 flex flex-col items-center gap-3">
              ...
              <button type="button" onClick={() => setImageUrl(null)} className="text-xs font-medium text-red-500 hover:text-red-700">Remove &amp; re-upload</button>
            </div>
          ) : (
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="flex flex-col gap-1.5"><label htmlFor="photo-type" className="text-sm font-medium text-zinc-700">Photo Type</label><select id="photo-type" value={photoType} onChange={(e) => setPhotoType(e.target.value as PhotoType)} className="h-10 w-full rounded-lg border border-zinc-200 bg-white px-3 text-sm text-zinc-900 focus:border-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-200">{PHOTO_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}</select></div>
            <div className="flex flex-col gap-1.5"><label htmlFor="photo-weight" className="text-sm font-medium text-zinc-700">Current Weight (kg)</label><input id="photo-weight" type="number" value={weight} onChange={(e) => setWeight(e.target.value)} placeholder="72" step={0.1} min={20} max={500} className="h-10 w-full rounded-lg border border-zinc-200 bg-white px-3 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-200" /></div>
            <div className="flex flex-col gap-1.5"><label htmlFor="photo-notes" className="text-sm font-medium text-zinc-700">Notes</label><input id="photo-notes" type="text" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Optional note" className="h-10 w-full rounded-lg border border-zinc-200 bg-white px-3 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-200" /></div>
          </div>
        </div>

        <div className="flex gap-3">
          <button type="submit" disabled={saving} className="rounded-lg bg-zinc-900 px-5 py-2.5 text-sm font-semibold text-white hover:bg-zinc-700 disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-900">{saving ? "Saving..." : "Save Progress Photo"}</button>
          <Link href="/progress/photos" className="inline-flex items-center px-4 py-2.5 text-sm font-medium text-zinc-500 hover:text-zinc-900">Cancel</Link>
        </div>
      </form>
    </div>
  );
