      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${difficultyColor(exercise.difficulty)}`}>
              {exercise.difficulty}
            </span>
          </div>
        </div>
        ...
        {detail?.alternatives ? (
          <div className="flex flex-wrap gap-2">
            {detail.alternatives.map((alt) => (
              <span key={alt} className="rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-1.5 text-xs font-medium text-zinc-700">
                {alt}
              </span>
            ))}
          </div>
